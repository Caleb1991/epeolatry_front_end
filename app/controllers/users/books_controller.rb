class Users::BooksController < ApplicationController
  def index
    if params.include?('title_filter')
      ...
      all_books = BookFacade.get_all_books(session[:user_id], params[:page])
      ...
      @books = BookFacade.get_all_books(session[:user_id], params[:page])
    end
  ...
  def show
    @book = BookFacade.get_single_book(session[:user_id], params[:id])
    @words = WordFacade.get_words_for_given_book(session[:user_id], params[:id])
  end
end
